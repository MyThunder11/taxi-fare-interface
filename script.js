let taxiFareApiUrl =
  "https://taxifare-2r6xryaxsa-ew.a.run.app/predict"; // replace with your API endpoint
const centralCoordinates = [-74.00597, 40.71427]; // starting position [lng, lat]

if (
  window.location.href.includes(
    "https://taxifare.lewagon.com"
  )
) {
...
const pickupAutocomplete = () => {
  ...
  geocoder.on("result", (event) => {
    ...
    document.querySelector("#pickup_latitude").value =
      coordinates[1];
    document.querySelector("#pickup_longitude").value =
      coordinates[0];
  });
};
...
const dropoffAutocomplete = () => {
  ...
  geocoder.on("result", (event) => {
    const coordinates = event["result"]["center"];
    document.querySelector("#dropoff_latitude").value =
      coordinates[1];
    document.querySelector("#dropoff_longitude").value =
      coordinates[0];
    const start = [
      parseFloat(
        document.querySelector("#pickup_longitude").value
  ...
  if (form) {
    form.addEventListener("submit", (e) => {
      ...
      const data = {
        ...
        pickup_datetime: document.getElementById(
          "pickup_datetime"
        ).value,
      };
      let query = [];
      ...
      fetch(url, {
        ...
        .then((data) => {
          ...
          const fareResult = document.getElementById(
            "predicted-fare"
          );
          const fare =
            Math.round(data["fare_amount"] * 100) / 100;
